...
| `ViewModels/MainViewModel.cs` | Root VM: `string Title`, `object? CurrentContent` (bound by ContentControl + DataTemplates), `SoftKeyFooterLayout FooterLayout` (Physical/Natural toggle), `ToggleFooterLayout()` method. Subscribes to `EvdevKeypadInput.KeyPressed`; dispatches keys to the `NavigationController` on the UI thread (`Dispatcher.UIThread.Post`). |
