...
| `Views/MainView.axaml` | Root layout: header (`Title`), body (`ContentControl Content="{Binding CurrentContent}"`), footer (6 soft-key cells via `ItemsControl` over `SoftKeys`). |
