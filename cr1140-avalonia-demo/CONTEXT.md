...
| `ViewModels/LedsViewModel.cs` | **LEDs demo** VM: live showcase of the `Cr1140.Avalonia.Leds` API driving real hardware. Cycles the RGB **status/indicator light** (`LedSysfs.SetRgb`, profile LED[0]) with F1, and the **keypad button backlight** colour (F2) + animation mode (F3, `LedMode` Solid/Dim/Pulse/Blink/Flash/Heartbeat) via the device-agnostic `Cr1140.Avalonia.Leds.KeypadBacklight` facade — `KeypadBacklight.For(profile)` drives the sysfs `*:kbd_backlight` PWM LED on the CR1140/CR1141 (animated) or the `com.ifm.Keyboard` D-Bus key backlights on the CR1102 (solid; `SupportsAnimation`=false, so the mode row reads "solid (fixed)"). A ~30 Hz `DispatcherTimer` calls `_keypad.Tick()` **only while the screen is open** (`Activate()`/`Deactivate()` from `NavigationController.OpenScreen`); both `Deactivate` and an `AppDomain.ProcessExit` hook call `_keypad.Off()`, so the keypad backlight is reset on screen exit **and** app teardown (the MCU-owned CR1102 keys would otherwise stay lit after the app quits) — the status light persists. The ctor asserts status=green + the keypad colour at app start. `Views/LedsView.axaml` shows two colour swatches (status \| keypad) via `HexToBrushConverter`, the mode name, and a sysfs-access hint. |
