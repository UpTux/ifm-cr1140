...
| `ViewModels/<Screen>ViewModel.cs` | Per-screen VM (Menu, Dashboard, BaleCounter, Knives, Wrapping, Telemetry, Settings, KeyEvents, Leds). Each exposes bindable properties and is mapped to its `Views/<Screen>View.axaml` via App-level `DataTemplate`s. |
