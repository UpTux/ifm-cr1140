...
| Type | Role |
|------|------|
| `static class Backlight` | Thin, typed reader/writer for the display backlight under `/sys/class/backlight/` — the .NET counterpart of the Rust `cr1140-hal` `sys` backlight functions (`set_backlight` / `read_backlight` / `backlight_max` / `list_backlights`). `Default` (=`"backlight"`, the CR1140 node name) and `MaxHint` (=400) constants; `Set(name, value)` / `Read(name)` (raw `/sys/class/backlight/<name>/brightness`), `Max(name)` (reads `max_brightness`), `ListBacklights()`, plus `SetPercent(name, 0..100)` / `ReadPercent(name)` percentage helpers that scale against `max_brightness` so an app can offer a simple 0–100 % operator brightness control without knowing the panel's raw range. Writes return `bool` and reads return `uint?`/`double?` — every call degrades off-device instead of throwing, like `LedSysfs`. New internal `Root` property (filesystem prefix, default empty = `/sys/class/backlight`) lets the emulator redirect writes to a temp tree so backlight changes are observable off-device. |
...
| `static class LedSysfs` | Thin, typed sysfs reader/writer — the .NET counterpart of the Rust `cr1140-hal` `sys` LED functions: `Name(Led)`, `Max(Led)`, `Set(name, value)` / `Read(name)` (raw `/sys/class/leds/<name>/brightness`), `SetTyped(Led, value)` (clamps to `Max`), `SetKbdBacklight(r, g, b)` (the three PWM channels as one RGB color), `ListLeds()`. Writes return `bool` and reads return `uint?` — every call degrades off-device (missing node / no permission) instead of throwing, like `ProcFs`. New internal `Root` property (filesystem prefix, default empty = `/sys/class/leds`) lets the emulator redirect writes to a temp tree so LED actuation is observable off-device. |
...
| Type | Role |
|------|------|
| `sealed class WindowKeypadInput : IKeypadInput, IDisposable` | Desktop keypad source. Maps physical keyboard (`F1`–`F6`, arrow keys, `Enter`/Return) and on-screen-button `Press(KeypadKey)`/`Release(KeypadKey)` calls into `KeypadKey`, deriving the same tap/double-tap/hold/holding gestures via the shared pure `KeyGestureDetector` on a 25 ms UI-thread `DispatcherTimer` (stopped when idle). `Attach(InputElement)` hooks a control's KeyDown/KeyUp; `static KeypadKey? Map(Key)` translates a physical Avalonia key. |
| `sealed class EmulatedDevice : IDisposable` | Off-device hardware shim. Creates a seeded temporary sysfs directory tree and redirects the internal `LedSysfs.Root` / `Backlight.Root` hooks at it, so an app's real `LedSysfs` / `Backlight` / `LedDriver` writes land in the temp tree and become observable off-device (the device code path is byte-identical — still file I/O). Exposes live `StatusColor` (RGB status light), `KbdColor` (RGB keypad backlight), `BacklightPercent`. `Dispose()` restores the roots to empty and deletes the temp tree. Single-instance (the roots are shared statics). |
| `sealed class EmulatorOptions` | Configuration: `PanelWidth` (default 800), `PanelHeight` (default 480), `Rotation` (`Cr1140.Avalonia.Output.DisplayRotation`, default `None`; 90/270 swap to the rotated logical size), `Title`, `ShowKeypad`, `ShowKeyboardHints`, and `KeyCaptions` (optional `Func<KeypadKey,string?>` giving a live caption per key — polled ~30 Hz, so it follows app state such as the soft-key footer layout). |
| `sealed class EmulatorWindow : Window` | The device bezel. Hosts the app's root view at the rotation-aware logical panel size (800×480); below it an on-screen keypad in the panel's **physical single-row layout** (`F6 F4 F2 · d-pad · F1 F3 F5`) wired to `WindowKeypadInput` via pointer events — each F-key keeps its fixed hardware label and shows a **live caption** (from `EmulatorOptions.KeyCaptions`) of the soft-key it currently triggers; plus a keypad-backlight tint, a **status-LED dot** (bottom-right, like the panel), and a screen-dim overlay reflecting `BacklightPercent` — all refreshed by a 33 ms poll of the `EmulatedDevice`. Physical keyboard is attached to the window. |
